...
for i in range(10):
    NIC.objects.get_or_create(
        nic_number=nics["nic_number"][i], delivery_date=nics["delivery_date"][i]
    )
...
for i in range(10):
    ...
    reader, _ = Reader.objects.get_or_create(
        user=user, title=readers["title"][i], nic=nic
    )
...
for i in range(30):
    author, _ = Author.objects.get_or_create(
        first_name=authors["first_name"][i], last_name=authors["last_name"][i]
    )
...
tags_ids = []
for i in range(8):
    tag, _ = Tag.objects.get_or_create(name=tags["name"][i])
...
with open(csv_file_path / "book.csv", "w") as csv_file:
    ...
    data = {
        ...
        "category": [
            fake.random.choice(["pr", "ar", "hi", "po", "ot"]) for _ in range(10)
        ],
...
for i in range(10):
    ...
    book_authors = [
        Author.objects.get(pk=author_id) for author_id in books["authors"][i]
    ]
...
with open(csv_file_path / "myread.csv", "w") as csv_file:
    ...
    data = {
        ...
        "end_read_date": [
            fake.date_between_dates(date.fromisoformat(start)) if p_r == 100 else None
...
# STATUS PERCENT
for status, per_range in [
    ("pending", "[0,0]"),
    ("reading", "[1,99]"),
    ("done", "[100,100]"),
]:
    StatusPercent.objects.get_or_create(
        read_status=status, percentage_read_range=per_range
    )
